...
const client = new tmi.Client({
  ...
  let reminderData = [];
  try {
    const jsonData = fs.readFileSync('/home/ubuntu/BOT/reminderslist.json', 'utf8');
...
client.on('message', async (channel, userstate, message, self) => {
  ...
  fs.readFile('/home/ubuntu/BOT/reminderslist.json', 'utf8', async (err, jsonData) => {
    if (err) {
      console.error('Error reading JSON file:', err);
      return;
    }
    ...
    if (remindcheck && !(remindcheck.length > 1)) {
      ...
      client.action(
        channel,
        `@${userstate["username"]}, reminder from: ${remindcheck[0].sender} - ${
          remindcheck[0].message
        } (${humanizeDuration(new Date().getTime() - Date.parse(remindcheck[0].time), {
          round: true,
        })} ago)`
      );
  ...
  if (afkcheck) {
          ...
          fs.readFile(
            ...
            function (err, data) {
              if (err) {
                ...
                client.action(
                  channel,
                  `Test number ${count} successful xD `
                );
    ...
    fs.readFile('/home/ubuntu/BOT/offlineonly.json', 'utf8', async(err, jsonData) => {
      if (err) {
        console.error('Error reading JSON file:', err);
        return;
      }
    ...
    if (command && channelsToCheck.includes(channel.replace("#","")) && liveChannels.includes(channel.replace("#",""))) {
    ...
    fs.readFile(customCommandsFilePath, "utf8", (err, jsonData) => {
      ...
      function getUserInfo(channel) {
        return got(`https://api.ivr.fi/v2/twitch/user?login=${channel.replace("#", "")}`, {
          responseType: "json",
      ...
      if (channelCommandsEntry) {
        ...
        if (commandToExecute) {
          ...
          console.log(`No custom command found with name "${commandName}" in channel "${channel}"`);
        }
      } else {
        console.log(`No custom commands found for channel "${channel}"`);
      }
    })
  }
